...
class Dictionary
{
	...
	bool ch; // логическая переменная, смыл которой заключается в описании функции setCh(): (функция, неоходимая для корректного ввода слов из файла в словарь(так как функция ввода слов из потока, используемая и при вводе слов из файла реализована с помощью методов AddWord и AddTranslate, то есть, если слов будет много, то и вызываться они будут неоднократно, а в них и содержатся возбуждения исключений, обработчик одного из которых описан ниже) как один из вариантов решения данной проблемы)

public:
	void setCh(); //описана выше()
